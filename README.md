[![Build status](https://ci.appveyor.com/api/projects/status/car80yfrdyeyvxda?svg=true)](https://ci.appveyor.com/project/stueber/doc-davinci-schulungen-de)

# DAVINCI 6 Schulungsunterlagen

Dies sind die Schulungsunterlagen für [DAVINCI](https://davinci.stueber.de). Die Dokumentation ist Open Source und wir haben sie mit [MkDocs](https://www.mkdocs.org) und [Material for MkDocs](https://squidfunk.github.io/mkdocs-material) realisiert. Push-Requests im master-branch werden in [AppVeyor](https://www.appveyor.com) getriggert, so dass jede Änderung automatisch veröffentlicht wird.
...
1. Erstelle einen lokalen Ordner für die Dokumentation, z.B. `c:\docs\davinci-schulungen`.

2. Starte die Eingabeaufforderung und wechsle in den Ordner `c:\docs\davinci-schulungen`.

3. Tippe den Befehl `git clone https://github.com/stuebersystems/doc.davinci-schulungen.de.git` ein, um das Repository zu klonen.
...
1. Öffne die URL `https://github.com/stuebersystems/doc.davinci-schulungen.de` in deinem Webbrowser

2. Klicke auf die Schaltfläche `Clone or download` und dann auf `Download ZIP`.

3. Entpacke das Zip-Archiv in einen lokalen Ordner Deiner Wahl, z.B. `c:\docs\davinci-schulungen`.
...
1. Starte die Eingabeaufforderung und wechsle in den Ordner `c:\docs\davinci-schulungen`.
